...
For the raster versions — a GitHub social preview, or anywhere SVG is not accepted —
`scripts/capture-evidence.sh` renders the SVGs and the latest Jaeger trace to PNG through
headless Chrome:

```bash
./scripts/capture-evidence.sh              # PNGs from the committed SVGs, no API calls
./scripts/capture-evidence.sh --regenerate # re-run the CLI first, then render
```

Output lands in `docs/images/png/`, which is gitignored: those files are derived from the
committed SVGs and regenerable on demand.
